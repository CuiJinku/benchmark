...
from userbenchmark.utils import REPO_PATH, add_path, get_output_dir, get_output_json, dump_output
...
class CoverageMode(torch.overrides.TorchFunctionMode):
    ...
    def check_func_in_APIs(self, func, *args):
        ...
        if (module_name, func_name) not in API_LIST:
            ...
            pretty_func_name = func_name.split('.')[-1] if '.' in func_name else func_name
            if pretty_func_name in HOMO_APIS:
                self.homonyms.add(func_name)
            # if pretty_func_name == 'sqrt':
            #     print(f"shape: {args[0].shape}")
            #     if args[0].shape == torch.Size([1]):
            #         print("Info: find one")
            #     # if arg0.
    ...
    def __torch_function__(self, func, types, args=(), kwargs=None):
        ...
        if kwargs is None:
            kwargs = {}
        self.check_func_in_APIs(func, *args)
...
def run(args: List[str]):
        ...
        try:
            ...
            with CoverageMode('', '') as coverage:
                try:
                    ...
                    if cfg.name in api_homonyms:
                        api_homonyms[cfg.name+"_eval"] = list(coverage.homonyms)
                    else:
                        api_homonyms[cfg.name] = list(coverage.homonyms)
    ...
    api_homonyms_fname = log_dir.joinpath("%s-api_homonyms.csv" % fname)
    with open(api_homonyms_fname, 'w') as f:
        for model_name, homonyms in api_homonyms.items():
            f.write(f"{model_name}:\n")
            for api in sorted(homonyms):
                f.write(f"    {api}\n")
